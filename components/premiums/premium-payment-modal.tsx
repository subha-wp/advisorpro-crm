...
interface PremiumPaymentDrawerProps {
  isOpen: boolean
  onClose: () => void
...
export function PremiumPaymentDrawer({
  isOpen,
  onClose,
  onPaymentSuccess,
  preSelectedPolicy,
}: PremiumPaymentDrawerProps) {
  ...
  const updateDueDatePreview = () => {
    ...
    if (isFullPayment && selectedPolicy.premiumMode) {
      ...
      if (premiumConfig) nextDueDate = addMonths(currentDueDate, premiumConfig.months)
    }
    setDueDatePreview({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedClient || !selectedPolicy) {
      toast({ title: "Error", description: "Please select a client and policy", variant: "destructive" })
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: "Success", description: result.message || "Premium payment recorded successfully" })
        onPaymentSuccess()
        handleClose()
      } else {
  ...
  const isFormValid =
    selectedClient && selectedPolicy && formData.paymentMode && formData.amountPaid
  ...
  return (
    <Drawer open={isOpen} onOpenChange={handleClose}>
      <DrawerContent className="rounded-t-2xl max-h-[95vh] flex flex-col">
        <DrawerHeader className="sticky top-0 bg-background z-10">
          <DrawerTitle>
            {preSelectedPolicy
              ? `Payment for Policy ${preSelectedPolicy.policyNumber}`
              : "Add Premium Payment"}
          </DrawerTitle>
          <DrawerDescription>
            {preSelectedPolicy
            ...
            {showClientSelection && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Search and select the client for this premium payment
                  </CardDescription>
                </CardHeader>
                <CardContent>
                ...
                <CardContent className="space-y-6">
                  <div className="bg-muted p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div><span className="text-muted-foreground">Client Name:</span> <span className="ml-2 font-medium">{selectedClient?.name}</span></div>
                      {selectedClient?.mobile && (
                        <div><span className="text-muted-foreground">Mobile:</span> <span className="ml-2">{selectedClient.mobile}</span></div>
                      )}
                      <div><span className="text-muted-foreground">Policy Number:</span> <span className="ml-2 font-medium">{selectedPolicy.policyNumber}</span></div>
                      <div><span className="text-muted-foreground">Insurer:</span> <span className="ml-2">{selectedPolicy.insurer}</span></div>
                      {selectedPolicy.planName && <div><span className="text-muted-foreground">Plan:</span> <span className="ml-2">{selectedPolicy.planName}</span></div>}
                      {selectedPolicy.nextDueDate && (
                        <div><span className="text-muted-foreground">Current Due Date:</span> <span className="ml-2">{format(new Date(selectedPolicy.nextDueDate), "dd MMM yyyy")}</span></div>
                      )}
                      {selectedPolicy.premiumAmount && (
                        <div><span className="text-muted-foreground">Premium Amount:</span> <span className="ml-2 font-medium">₹{selectedPolicy.premiumAmount.toLocaleString()}</span></div>
                      )}
                    </div>
                  </div>
                      ...
                      <AlertDescription>
                        <div className="space-y-1">
                          <div className="font-medium">
                            {dueDatePreview.isFullPayment ? "Full Payment - Due Date Will Be Updated" : "Partial Payment - Due Date Unchanged"}
                          </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input id="paymentDate" type="date" value={formData.paymentDate} onChange={(e) => handleInputChange("paymentDate", e.target.value)} required />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input id="amountPaid" type="number" step="0.01" placeholder="0.00" value={formData.amountPaid} onChange={(e) => handleInputChange("amountPaid", e.target.value)} className="pl-10" required />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="receiptNumber" placeholder="Receipt number" value={formData.receiptNumber} onChange={(e) => handleInputChange("receiptNumber", e.target.value)} />
                    </div>
                  </div>
                  ...
                  {formData.paymentMode === "CHEQUE" && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <Input id="chequeNumber" placeholder="Cheque number" value={formData.chequeNumber} onChange={(e) => handleInputChange("chequeNumber", e.target.value)} />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="bankName">Bank Name</Label>
                        <Input id="bankName" placeholder="Bank name" value={formData.bankName} onChange={(e) => handleInputChange("bankName", e.target.value)} />
                      </div>
                    </div>
                  )}
                  ...
                  {(formData.paymentMode === "ONLINE" ||
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <Input id="transactionId" placeholder="Transaction ID" value={formData.transactionId} onChange={(e) => handleInputChange("transactionId", e.target.value)} />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="bankName">Bank Name</Label>
                        <Input id="bankName" placeholder="Bank name" value={formData.bankName} onChange={(e) => handleInputChange("bankName", e.target.value)} />
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <IndianRupee className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                          <Input id="lateFee" type="number" step="0.01" placeholder="0.00" value={formData.lateFee} onChange={(e) => handleInputChange("lateFee", e.target.value)} className="pl-10" />
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <IndianRupee className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                          <Input id="discount" type="number" step="0.01" placeholder="0.00" value={formData.discount} onChange={(e) => handleInputChange("discount", e.target.value)} className="pl-10" />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-muted p-4 rounded-lg">
                      ...
                      <div className="text-sm text-muted-foreground mt-1">
                        Amount Paid + Late Fee - Discount = ₹{formData.amountPaid || 0} + ₹{formData.lateFee || 0} - ₹{formData.discount || 0}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea id="remarks" placeholder="Additional notes or comments..." value={formData.remarks} onChange={(e) => handleInputChange("remarks", e.target.value)} rows={3} />
                  </div>
                </CardContent>
              </Card>
            )}
        ...
        {/* Sticky footer */}
        {selectedPolicy && (
          <DrawerFooter className="sticky bottom-0 bg-background border-t p-4 flex justify-end space-x-4">
            <DrawerClose asChild>
              <Button type="button" variant="outline">
                Cancel
              </Button>
            </DrawerClose>
            <Button type="submit" form="form" disabled={!isFormValid || isSubmitting}>
